...
const Market = () => {
    ...
    const [url, setUrl] = useState(
        `${import.meta.env.VITE_API_BASE_URL}/api/traders/`
    );
    ...
    useEffect(() => {
        const fetchTraders = async () => {
            ...
            try {
                ...
                console.log(url);
                console.log(nextUrl);
                setTraders(response.data.results || []);
    ...
    return (
        <section
            ...
            className="container mx-auto flex flex-col py-2 gap-y-4 px-2 relative"
        >
            <AuthNavbar />
            ...
            <div className="py-8 mt-4 flex flex-col lg:px-12 w-full">
                ...
                <div className="flex flex-col pt-8 mt-8 gap-y-4">
                    ...
                    {/* pagination button */}
                    <div className="w-full flex items-center justify-center gap-x-12 py-4 mt-4 ">
                        {traders.length > 0 && (
                            <>
                                <button
                                    onClick={() =>
                                    ...
                                    disabled={!prevUrl}
                                    className="w-fit px-4 sm:px-8 py-2 sm:py-3 rounded-sm bg-primary text-sm text-black font-semibold hover:scale-105 transition-all duration-300 cursor-pointer disabled:bg-whyCard disabled:text-gray-500 disabled:cursor-not-allowed"
                                >
                                    Previous
                                </button>
                ...
                <div className="absolute inset-0 bg-black/25 backdrop-blur-sm h-full flex items-center justify-center px-8">
                    <div className="w-full sm:w-[70%] lg:w-[60%] xl:w-[50%] p-8 bg-deposit min-h-[400px] rounded-2xl flex flex-col gap-4 items-center justify-center text-center">
                        ...
                        <h3 className="text-xl font-bold text-black">
                            Copy Request Pending
                        </h3>
                        <span className="text-pretty text-black text-sm font-normal">
                            The trader has received your copy request. Your
                            vault will start copying them as soon as your
                            request is accepted
                        </span>
                    </div>
                </div>
