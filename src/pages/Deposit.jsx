...
const Deposit = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors(
        error.response?.data || { receipt: ["Failed to submit receipt"] }
      );
      setLoading(false);
    }
  };
      ...
      <div className="w-full lg:w-[60%] flex flex-col py-2 ">
        ...
        <div className="w-full border-1 border-primary flex flex-col gap-y-2 rounded-lg px-6 py-6">
          ...
          <div className="w-full flex mt-4 py-4 justify-start gap-2 flex-col">
            ...
            {errors.receipt && (
              <p className="text-red-500 text-sm mt-1">{errors.receipt[0]}</p>
            )}
            ...
            <div className=" w-full flex flex-col gap-3">
              <h3 className="text-lg font-semibold text-primary">
                ...
                <span className=" mb-2 font-normal">
                  While most payments are tracked automatically, submitting a
                  screenshot helps us:
                </span>
              </h3>
              <div className="px-3 md:px-3 w-full flex flex-col gap-y-2">
                <div className="flex items-center gap-x-3">
                  ...
                  <span className="text-sm font-semibold text-primary">
                    Speed up transaction matching during high traffic
                  </span>
                </div>
                <div className="flex items-center gap-x-3">
                  <div className="rounded-full p-1 bg-primary"></div>
                  <span className="text-sm font-semibold text-primary">
                    Protect against fraud or mistaken deposits
                  </span>
                </div>
                <div className="flex items-center gap-x-3">
                  <div className="rounded-full p-1 bg-primary"></div>
                  <span className="text-sm font-semibold text-primary">
                    Resolves issues quickly if something goes wrong
                  </span>
                </div>
                <div className="flex items-center gap-x-3">
                  <div className="rounded-full p-1 bg-primary"></div>
                  <span className="text-sm font-semibold text-primary">
                    Track third-party wallet activity more reliably
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2 py-2 mt-2">
                ...
                <h3 className="text-sm font-semibold text-red-500">
                  Only send <span>{selectAsset.name} </span>to this address on{" "}
                  <span>{selectAsset.network} </span> network. Sending other
      ...
      >
        <div className="fixed inset-0 z-10 w-screen  overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4">
            <DialogPanel
              transition
              className="w-[400px] md:w-[450px] lg:w-[500px] rounded-xl bg-white/25 p-6 backdrop-blur-2xl duration-300 ease-out data-closed:transform-[scale(95%)] data-closed:opacity-0"
            >
              <DialogTitle
                as="h3"
              ...
              <p className="mt-2 text-sm font-normal text-gray-300">
                To help us verify your transaction quickly, please upload a
                screenshot of your payment receipt. Ensure the screenshot
                clearly shows the transaction details (amount, date, and wallet
                address). Thank you for your cooperation.
              </p>
              <div className="mt-4">
                <Button
                  className="inline-flex items-center gap-2 rounded-md bg-black px-3 py-1.5 text-sm/6 font-semibold text-white shadow-inner shadow-white/10 focus:not-data-focus:outline-none data-focus:outline data-focus:outline-white data-hover:bg-gray-600 data-open:bg-gray-700"
                  onClick={close}
                >
                  Got it, thanks!
                </Button>
